...
public class TreeCutting
{
    ...
    public static void on_config(List<String> universal_logs)
    {
        ...
        try {
            universal_logs.forEach(rls -> {
                ...
                universal_logs_.add(BuiltInRegistries.BLOCK.get(rl));
            });
        } catch (Throwable ex) {
            Auxiliaries.logError("Unexpected exception parsing universal log blocks: " + ex.getMessage());
        }
        if (!universal_logs_.isEmpty()) {
            Auxiliaries.logger().info("Tree cutting: Universal logs: " +
                    universal_logs_.stream().map(Block::toString).collect(Collectors.joining(", ")) + ".");
        }
    }
    ...
    private static boolean isSameLog(BlockState a, BlockState b)
    {
        ...
        return (ba == bb)
                || (universal_logs_.contains(ba) && isLog(b))
                || (universal_logs_.contains(bb) && isLog(a))
                || (universal_logs_.contains(ba) && universal_logs_.contains(bb));
    }
    ...
    private static List<BlockPos> findBlocksAround(final Level world, final BlockPos centerPos,
                                                   final BlockState leaf_type_state, final Set<BlockPos> checked,
                                                   int recursion_left)
    {
    ...
    public static int chopTree(Level world, BlockState broken_state, BlockPos startPos,
                               int max_blocks_to_break, boolean without_target_block)
    {
        ...
        // Initial upward and lateral search
        {
