...
{
    /**
     * Registry provider used where modern APIs require a HolderLookup.Provider for
     * (de)serialization of ItemStacks / registries. Cached to avoid repeated expensive creation.
     */
    private static final HolderLookup.Provider REGISTRY_PROVIDER = VanillaRegistries.createLookup();
    ...
    public static IItemHandler itemhandler(Level world, BlockPos pos, @Nullable Direction side)
    {
        ...
        // NeoForge: block capabilities are accessed via Capabilities.ItemHandler.BLOCK
        // use Level.getCapability for block capabilities (context: BlockPos + side)
        IItemHandler ih = world.getCapability(Capabilities.ItemHandler.BLOCK, pos, side);
    ...
    public static IItemHandler itemhandler(Entity entity, @Nullable Direction side)
    {
        ...
        // NeoForge: query entity capability via Capabilities.ItemHandler.ENTITY (returns nullable IItemHandler)
        IItemHandler ih = entity.getCapability(Capabilities.ItemHandler.ENTITY);
        if(ih!=null) return ih;
    ...
    public static ItemStack insert(BlockEntity te, @Nullable Direction side, ItemStack stack, boolean simulate)
    {
        ...
        // NeoForge: use Level.getCapability via block entity's level and pos
        IItemHandler hnd = (te.getLevel() != null) ? te.getLevel().getCapability(Capabilities.ItemHandler.BLOCK, te.getBlockPos(), side) : null;
        if(hnd == null) {
    ...
    public static class MappedItemHandler implements IItemHandler
    {
        ...
        public ItemStack insertItem(int slot, @Nonnull ItemStack stack, boolean simulate)
        {
            ...
            if(!sst.isEmpty()) {
                ...
                // use new ItemStack comparison for components-aware equality
                if(!ItemStack.isSameItemSameComponents(stack, sst)) return stack;
        ...
        // NeoForge no longer uses LazyOptional; return Optional<IItemHandler> to preserve wrapper-like behavior

        public static Optional<IItemHandler> createGenericHandler(Container inv, BiPredicate<Integer, ItemStack> extraction_predicate, BiPredicate<Integer, ItemStack> insertion_predicate, BiConsumer<Integer, ItemStack> insertion_notifier, BiConsumer<Integer, ItemStack> extraction_notifier)
        { return Optional.of(new MappedItemHandler(inv, extraction_predicate, insertion_predicate, insertion_notifier, extraction_notifier)); }

        public static Optional<IItemHandler> createGenericHandler(Container inv, BiPredicate<Integer, ItemStack> extraction_predicate, BiPredicate<Integer, ItemStack> insertion_predicate, BiConsumer<Integer, ItemStack> insertion_notifier, BiConsumer<Integer, ItemStack> extraction_notifier, List<Integer> slot_map)
        { return Optional.of(new MappedItemHandler(inv, slot_map, extraction_predicate, insertion_predicate, insertion_notifier, extraction_notifier)); }

        public static Optional<IItemHandler> createGenericHandler(Container inv, BiPredicate<Integer, ItemStack> extraction_predicate, BiPredicate<Integer, ItemStack> insertion_predicate, List<Integer> slot_map)
        { return Optional.of(new MappedItemHandler(inv, slot_map, extraction_predicate, insertion_predicate)); }

        public static Optional<IItemHandler> createGenericHandler(Container inv, BiPredicate<Integer, ItemStack> extraction_predicate, BiPredicate<Integer, ItemStack> insertion_predicate)
        { return Optional.of(new MappedItemHandler(inv, extraction_predicate, insertion_predicate)); }

        public static Optional<IItemHandler> createGenericHandler(Container inv)
        { return Optional.of(new MappedItemHandler(inv)); }

        public static Optional<IItemHandler> createExtractionHandler(Container inv, BiPredicate<Integer, ItemStack> extraction_predicate, List<Integer> slot_map)
        { return Optional.of(new MappedItemHandler(inv, slot_map, extraction_predicate, (i, s)->false)); }

        public static Optional<IItemHandler> createExtractionHandler(Container inv, BiPredicate<Integer, ItemStack> extraction_predicate)
        { return Optional.of(new MappedItemHandler(inv, extraction_predicate, (i, s)->false)); }

        public static Optional<IItemHandler> createExtractionHandler(Container inv, Integer... slots)
        { return Optional.of(new MappedItemHandler(inv, Arrays.asList(slots), (i, s)->true, (i, s)->false)); }

        public static Optional<IItemHandler> createExtractionHandler(Container inv)
        { return Optional.of(new MappedItemHandler(inv, (i, s)->true, (i, s)->false)); }

        public static Optional<IItemHandler> createInsertionHandler(Container inv, BiPredicate<Integer, ItemStack> insertion_predicate, List<Integer> slot_map)
        { return Optional.of(new MappedItemHandler(inv, slot_map, (i, s)->false, insertion_predicate)); }

        public static Optional<IItemHandler> createInsertionHandler(Container inv, Integer... slots)
        { return Optional.of(new MappedItemHandler(inv, Arrays.asList(slots), (i, s)->false, (i, s)->true)); }

        public static Optional<IItemHandler> createInsertionHandler(Container inv, BiPredicate<Integer, ItemStack> insertion_predicate)
        { return Optional.of(new MappedItemHandler(inv, (i, s)->false, insertion_predicate)); }

        public static Optional<IItemHandler> createInsertionHandler(Container inv)
        { return Optional.of(new MappedItemHandler(inv, (i, s)->false, (i, s)->true)); }
    }
        ...
        public ItemStack extract(final ItemStack request_stack)
        {
            ...
            for(int i=0; i<size_; ++i) {
                ...
                if((!stack.isEmpty()) && (areItemStacksIdentical(stack, request_stack))) {
                    if (stack.has(DataComponents.CUSTOM_DATA)) {  // Проверка наличия компонента
                        final CompoundTag nbt = Objects.requireNonNull(stack.get(DataComponents.CUSTOM_DATA)).copyTag();  // Явная проверка null
                        int n = nbt.size();
    ...
    public static class StorageInventory implements Container, Iterable<ItemStack>
    {
        ...
        { return ContainerHelper.saveAllItems(nbt, stacks_, REGISTRY_PROVIDER); }

        public CompoundTag save(CompoundTag nbt, boolean save_empty)
        { return ContainerHelper.saveAllItems(nbt, stacks_, save_empty, REGISTRY_PROVIDER); }
        ...
        { stacks_.clear(); ContainerHelper.loadAllItems(nbt, stacks_, REGISTRY_PROVIDER); return this; }
        ...
        public boolean isItemValid(int targetSlot, ItemStack stack) {
            return validator_.test(targetSlot, stack);
        }
    }
    ...
    public static NonNullList<ItemStack> readNbtStacks(CompoundTag nbt, String key, int size)
    {
        ...
        ContainerHelper.loadAllItems(stacknbt, stacks, REGISTRY_PROVIDER);
        return stacks;
    }
    ...
    public static NonNullList<ItemStack> readNbtStacks(CompoundTag nbt, int size)
    {
        ...
        ContainerHelper.loadAllItems(nbt, stacks, REGISTRY_PROVIDER);
        return stacks;
    }
    ...
    public static CompoundTag writeNbtStacks(CompoundTag nbt, String key, NonNullList<ItemStack> stacks, boolean omit_trailing_empty)
    {
        ...
        ContainerHelper.saveAllItems(stacknbt, stacks, REGISTRY_PROVIDER);
