...
public class RfEnergy {
    ...
    public static int feed(Level world, BlockPos pos, @Nullable Direction side, int rf_energy) {
        if (world == null) return 0;System.out.println("RfEnergy.feed: checking " + pos + " side=" + side);

        final BlockEntity te = world.getBlockEntity(pos);
        if (te == null) return 0;

        // ✅ Новый способ получения Capability в NeoForge 1.21.1
        final IEnergyStorage es = world.getCapability(
    ...
    public static class Battery implements IEnergyStorage {
        ...
        public Battery(int capacity, int transfer_rate) { this(capacity, transfer_rate, transfer_rate, 0); }

        public Battery(int capacity, int charge_rate, int discharge_rate) { this(capacity, charge_rate, discharge_rate, 0); }
        ...
        public int getChargeRate() { return charge_rate_; }

        public int getDischargeRate() { return discharge_rate_; }

        public boolean isEmpty() { return energy_ <= 0; }

        public boolean isFull() { return energy_ >= capacity_; }

        public int getSOC() { return (int) Mth.clamp((100.0 * energy_ / capacity_ + .5), 0, 100); }

        public int getComparatorOutput() { return (int) Mth.clamp((15.0 * energy_ / capacity_ + .2), 0, 15); }
        ...
        public Battery load(CompoundTag nbt) { return load(nbt, "Energy"); }
        ...
        public CompoundTag save(CompoundTag nbt) { return save(nbt, "Energy"); }

        public Optional<IEnergyStorage> createEnergyHandler() {
        ...
        public int receiveEnergy(int feed_energy, boolean simulate) {
            ...
            if (!simulate) {
                energy_ += e;
                System.out.println("Energy received in Battery: " + e + ", stored: " + energy_); // Добавляем отладку
            }
            return e;
        }
        ...
        @Override
        public int getEnergyStored() { return energy_; }

        @Override
        public int getMaxEnergyStored() { return capacity_; }

        @Override
        public boolean canExtract() { return discharge_rate_ > 0; }

        @Override
        public boolean canReceive() { return charge_rate_ > 0; }
    }
}
